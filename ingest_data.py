...
def extract_data(url: str) -> None:
    ...
    if url.endswith(".csv.gz"):
        outfile_name = "green_tripdata_2019-09.csv.gz"
...
def transform_data(df):
    ...
    print(f"\npost: rows: {len(new_df)}\npost:missing passenger count: {new_df['passenger_count'].isin([0]).sum()}")

    return new_df


@flow(log_prints=True)
def ingest_data(outfile_name: str, table_name: str) -> None:
    """
    Read in the dataFrame
    """
    # read in csv in batches
    batch_iter = pd.read_csv(
        outfile_name, iterator=True, chunksize=100000, compression="gzip"
    )
    df = next(batch_iter)
...
def main_flow():
    ...
    csv_url = "~/code_projx/de-practice/green_tripdata_2019-09.csv.gz"
